...
const (
	...
	UserService_UpdateStatus_FullMethodName   = "/user.UserService/UpdateStatus"
	UserService_StreamPresence_FullMethodName = "/user.UserService/StreamPresence"
)
...
type UserServiceClient interface {
	...
	StreamPresence(ctx context.Context, in *Empty, opts ...grpc.CallOption) (grpc.ServerStreamingClient[PresenceEvent], error)
}
...
func (c *userServiceClient) StreamPresence(ctx context.Context, in *Empty, opts ...grpc.CallOption) (grpc.ServerStreamingClient[PresenceEvent], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &UserService_ServiceDesc.Streams[0], UserService_StreamPresence_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[Empty, PresenceEvent]{ClientStream: stream}
...
type UserService_StreamPresenceClient = grpc.ServerStreamingClient[PresenceEvent]
...
type UserServiceServer interface {
	...
	StreamPresence(*Empty, grpc.ServerStreamingServer[PresenceEvent]) error
...
func (UnimplementedUserServiceServer) StreamPresence(*Empty, grpc.ServerStreamingServer[PresenceEvent]) error {
...
func _UserService_StreamPresence_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(Empty)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(UserServiceServer).StreamPresence(m, &grpc.GenericServerStream[Empty, PresenceEvent]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type UserService_StreamPresenceServer = grpc.ServerStreamingServer[PresenceEvent]
	...
	Streams: []grpc.StreamDesc{
		{
			...
			Handler:       _UserService_StreamPresence_Handler,
			ServerStreams: true,
		},
	},
